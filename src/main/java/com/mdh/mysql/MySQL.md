...
##  触发时机
~~~
1.后台定时刷新（Master Thread）：
    InnoDB 的主后台线程（Master Thread）会每秒或每 10 秒将一部分脏页异步刷新到磁盘。这是为了保持脏页数量在一个可控范围内。
2.脏页比例过高（Dirty Page Ratio）：
    当 Buffer Pool 中脏页的数量超过一定阈值时，会触发刷新。这个阈值由参数 innodb_max_dirty_pages_pct 控制（默认通常是 75%）。这意味着 Buffer Pool 中最多允许 75% 的页是脏的，超过这个比例，系统会加快刷盘速度。
3.Buffer Pool 空间不足（LRU 淘汰）：
    当 Buffer Pool 满了，需要加载新数据页时，必须根据 LRU 算法淘汰旧页。如果被淘汰的页是脏页，则必须先将其刷新到磁盘（变为干净页），然后才能释放空间。
4.Redo Log 写满（Checkpoint 机制）：
    这是为了保证崩溃恢复的能力。Redo Log 是循环写入的，当 Redo Log 写满时，必须将对应的脏页刷新到磁盘，推进检查点（Checkpoint），从而腾出 Redo Log 空间供新事务使用。
正常关闭数据库：
    当 MySQL 正常关闭时，会将所有脏页全部刷新到磁盘（Sharp Checkpoint），以确保下次启动时数据的一致性。
~~~

## 执行线程
~~~
在 MySQL 5.6 之后，刷新工作主要由专门的后台线程负责，以减少对主线程的干扰：
    Page Cleaner Thread：专门负责脏页刷新的线程。它将刷盘任务从 Master Thread 中独立出来，可以更高效地处理 I/O 请求。
    Master Thread：在旧版本或特定情况下，主线程也会负责部分刷盘工作。
~~~

## 核心机制
~~~
当决定刷新一个脏页时，InnoDB 并不是简单地把内存数据覆盖到磁盘文件上，而是有一套严密的流程：
    1.选择脏页：Page Cleaner 线程从 Flush 链表（专门存储脏页的链表）中选取一批脏页。
    2.Doublewrite（双写）保护：
        这是 InnoDB 防止页断裂（Partial Page Write）的关键机制。
        因为操作系统页大小（通常 4KB）小于 InnoDB 页大小（16KB），如果在写磁盘过程中发生宕机，可能导致磁盘上的页只写了一半，造成数据损坏。
        流程：InnoDB 先将脏页写入内存中的 Doublewrite Buffer，然后顺序写入磁盘上的 Doublewrite 共享表空间，最后才写入各自的数据文件（.ibd）。如果写入数据文件失败，重启时可以通过 Doublewrite 备份恢复。
    3.写入磁盘：将数据真正写入 .ibd 文件。
    4.标记为干净：写入成功后，该页在 Buffer Pool 中被标记为“干净页”，并从 Flush 链表中移除。
~~~
