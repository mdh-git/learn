...
1. 循环时间长,开销大(允许多线程互相修改)
    getAndAddInt方法下 do while
        解決：可以使用自适应自旋锁，即在多次操作失败后逐渐加长自旋锁或者放弃自旋锁转为阻塞锁
2. 只能保证一个共享变量的原子操作
    加锁可以保证一段变量的原子操作
        解决： 1. 可以使用锁机制来保证整个复合操作的原子性 。
                    使用Synchronized关键字 或者 ReentrantLock来保证多个变量的复合操作的原子性。
              2. 可以使用类似于事务的方式老保证多个变量的复合操作的原子性。
                    AtomicReference类，多个变量封装到一个对象中。
3. ABA问题 原子引用更新
    CAS算法实现一个重要前提需要取出内存中某个时刻的数据并在当下时刻比较替换,
    那么在这个时间差内会导致数据的变化。
        解決：增加版本号， AtomicStampedReference
